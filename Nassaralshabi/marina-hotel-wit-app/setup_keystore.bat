@echo off
chcp 65001 > nul

REM Marina Hotel - Keystore Setup Script for Windows
echo 🔑 Marina Hotel - Setting up Keystore...
echo ==========================================

REM Check if release.keystore already exists
if exist release.keystore (
    echo ⚠️  Keystore file already exists: release.keystore
    set /p overwrite="Do you want to overwrite it? (Y/N): "
    if /i not "%overwrite%"=="Y" (
        echo ❌ Cancelled. Keystore not created.
        pause
        exit /b 0
    )
)

echo 📝 Creating keystore from base64...

REM Create temporary base64 file
echo  > keystore_b64.tmp

REM Use PowerShell to decode base64
powershell -command "[Convert]::FromBase64String((Get-Content keystore_b64.tmp)) | Set-Content release.keystore -Encoding Byte"

if exist release.keystore (
    echo ✅ Keystore created successfully: release.keystore
    del keystore_b64.tmp
    
    echo.
    echo 📋 Keystore Information:
    echo    File: release.keystore
    echo    Alias: marina-hotel-key
    echo    Password: Marina2024!SecureKey789
    echo    Key Password: HotelApp@2024#Strong456
    echo.
    echo 🚀 You can now build release APK with:
    echo    gradlew.bat assembleRelease
    echo.
    echo 🎉 Setup completed successfully!
    echo 🔐 Remember: Keep your keystore secure and never commit it to Git!
) else (
    echo ❌ Failed to create keystore
    if exist keystore_b64.tmp del keystore_b64.tmp
)

pause