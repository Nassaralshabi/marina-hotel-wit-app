...
class EmployeesAdapter(private val listener: EmployeeListener) : RecyclerView.Adapter<EmployeesAdapter.EmployeeViewHolder>() {
    private val items = mutableListOf<EmployeeUi>()
    ...
    fun submitList(data: List<EmployeeUi>) {
        items.clear()
        items.addAll(data)
    ...
    inner class EmployeeViewHolder(private val binding: ItemEmployeeBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: EmployeeUi) {
            ...
            binding.employeeStatus.text = if (item.isActive) "نشط" else "موقوف"
